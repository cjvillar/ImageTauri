...
fn image_change_format(file_path: &str, save: bool, img_format: &str) -> Result<DynamicImage, String> {
    // check file exists
    ...
    if save {
        ...
        let save_result = match img_format {
            "PNG" => img.save(&new_file_path).map_err(|e| format!("Failed to save image as PNG: {}", e)),
            "JPEG" => img.save(&new_file_path).map_err(|e| format!("Failed to save image as JPEG: {}", e)),
            "BMP" => img.save(&new_file_path).map_err(|e| format!("Failed to save image as BMP: {}", e)),
            _ => return Err(format!("Unsupported image format: {}", img_format)),
        };
        ...
        img.write_to(&mut Cursor::new(&mut bytes), match img_format {
